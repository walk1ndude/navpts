...
public:
	...
	// constructors
    inline PVFilter(cv::Vec2f state, cv::Matx22f var)
		: state(state), var(var)
	{
	}

	inline PVFilter(double pose, double speed)
        : state(cv::Vec2f(pose,speed)), var(cv::Matx22f(0,0,0,0))
	{
	}

	inline PVFilter(double pose)
        : state(cv::Vec2f(pose,0)), var(cv::Matx22f(0,0,0,1e10))
	{
	}

	inline PVFilter()
        : state(cv::Vec2f(0,0)), var(cv::Matx22f(1e10,0,0,1e10))
	{
	}
	...
	inline void observePose(double obs, double obsVar)
	{
		...
		*/
        ...
        cv::Vec2f K = var.row(0) / (obsVar + var(0,0));	//K is first col = first row of var.
		state = state + K * (obs - state[0]);
        cv::Matx22f tmp(1-K[0], 0, -K[1], 1);
		var = tmp * var;
	}
	...
	inline void observeSpeed(double obs, double obsVar)
	{
		...
		*/
        ...
        cv::Vec2f K = var.row(1) / (obsVar + var(1,1));	//K is second col = second row of var.
		state = state + K * (obs - state[1]);
        cv::Matx22f tmp(1, -K[0], 0, 1-K[1]);
		var = tmp * var;
	}

	// predict
	// calculates prediction variance matrix based on gaussian acceleration as error.
	inline void predict(double ms, double accelerationVar, cv::Vec2f controlGains = cv::Vec2f(0,0), double coVarFac = 1, double speedVarFac = 1)
	{
		/* MATLAB:
		G = [1 ms/1000; 0 1];
	...
	inline void predict(double ms, cv::Vec3f vars, cv::Vec2f controlGains = cv::Vec2f(0,0))
	{
		/* MATLAB:
		G = [1 ms/1000; 0 1];
		E = [((ms/1000)^2)/2; ms/1000]; % assume normal distributed constant ACCELERATION.
